...
export class PlaceListingsComponent implements OnInit {
  ...
  keywords: string = '';
  places: Array<Place> = [];
  constructor(public route : ActivatedRoute, private router: Router, private placeListingsServ: PlaceListingsService, private locationServ: LocationService) { 
  ...
  async searchPlaces(){
    ...
    this.places = [];
    this.router.navigate(['/home/' + this.keywords])
